...
def trova_ultima_categoria(origin, key, origin_neg):
    temp = key.split(";")
    ult_cat = temp[0]
    ult_sott_cat = temp[1]
...
#         print(sottocategoria)
        if sottocategoria[0] == ult_sott_cat:
            ...
            ultimo_negozio = list(origin_neg[key])[-1]
            page_ultimo_store = (((origin_neg[key])[ultimo_negozio])[2])[0]    
